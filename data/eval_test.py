...
def Eval_Video(mode=None,mcp=None):
    ...
    for i in range(len(data)):
        current_video=data[i].split(os.path.sep)[8].split('.')[0]
        predict_videoname=data[i].split(os.path.sep)[8]
        print(current_video)
        pic_path=glob.glob(os.path.join('/home/sk49/workspace/dataset/QIYI_FACE/Test/face_crop','*jpg'))
        final_list=[]
        if(len(pic_path)!=0):
            for x in pic_path:
...
def get_video(version='01'):
    video_list=[]
    for x in glob.glob(os.path.join('/home/sk49/workspace/dataset/character_recog_iqiyi/test/IQIYI_VID_TEST','*mp4')):
        video_list.append(x)
