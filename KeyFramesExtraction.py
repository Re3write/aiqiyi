...
def extraction(len_window, video_dir, video_name, output_dir):
    print("Video :" + video_dir+video_name)
    ...
    if USE_TOP_ORDER:
        ...
        for keyframe in frames[:NUM_TOP_FRAMES]:
            name = video_name + "_" + str(keyframe.id) + ".jpg"
    ...
    if USE_THRESH:
        ...
        for i in range(1, len(frames)):
            if (rel_change(np.float(frames[i - 1].value), np.float(frames[i].value)) >= THRESH):
                ...
                name = video_name + "_" + str(frames[i].id) + ".jpg"
    ...
    if USE_LOCAL_MAXIMA:
        ...
        for i in frame_indexes:
            name = video_name + "_" + str(frames[i - 1].id) + ".jpg"
            # print(output_dir+name)
            cv2.imwrite(output_dir + name, frames[i - 1].frame)
...
if __name__ == '__main__':
    ...
    time1 = time.time()
    len_window = 16
    video_dir = "./data_view/video/"
    output_dir = "./data_view/frame/"
    for video_name in os.listdir(video_dir):
        extraction(len_window, video_dir, video_name, output_dir)

    time2 = time.time()
    print("done! ",time2-time1)
    # video_path = "./data_view/IQIYI_VID_TRAIN_0091197.mp4"
    # # Directory to store the processed frames
    # # dir = sys.argv[2]
    # output_dir = "./data_view/"
